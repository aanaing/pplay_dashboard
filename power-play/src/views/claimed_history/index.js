...
const Index = () => {
  ...
  const [loadHistoriesWithDate, resultWithDate] = useLazyQuery(
    CLAIM_HISTORIESWITHDATE
  );
  ...
  useEffect(() => {
    if (resultWithDate.data) {
      ...
      setCount(
        Number(resultWithDate.data?.claim_history_aggregate.aggregate.count)
      );
      let tempData = resultWithDate.data.claim_history;
      let temp = tempData?.map((d) => {
        return {
          id: d.id,
          status: d.claim_status ? "true" : "false",
          point_used: d.loyalty_points_used,
          claimer: d.user.username,
    ...
    <div>
      ...
      >
        <FormControl sx={{ width: 300 }}>
          <TextField
            id="outlined-search"
            label="Search by Claimer's Name"
            type="search"
            value={search}
            onChange={(e) => {
          ...
          <Table stickyHeader aria-label="sticky table">
            <TableHead>
              <TableRow>
                ...
                <TableCell style={{ minWidth: 60 }}>Status</TableCell>
                <TableCell style={{ minWidth: 70 }}>Point Used</TableCell>
                <TableCell style={{ minWidth: 70 }}>Claimer</TableCell>
                <TableCell style={{ minWidth: 70 }}>Loyalty Product</TableCell>
              ...
              {histories?.map((row) => {
                return (
                  <TableRow hover role="checkbox" tabIndex={-1} key={row.id}>
                    ...
                    <TableCell>{row.claim_status ? "true" : "false"}</TableCell>
                    <TableCell>{row.loyalty_points_used}</TableCell>
                    <TableCell>
                    ...
                    <TableCell>
                      <Link to={`/loualty_product/${row?.loyalty_product.id}`}>
                        {row?.loyalty_product.name}
                      </Link>
                    </TableCell>
          ...
          >
            <Button>
              <CSVLink
                ...
                data={data}
                headers={headers}
                filename={`claimed-histories-${new Date().toISOString()}.csv`}
              >
                Export
              </CSVLink>
            </Button>
